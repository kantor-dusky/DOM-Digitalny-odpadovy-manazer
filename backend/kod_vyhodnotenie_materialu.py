...
# PostgreSQL pripojenie (uprav podľa seba)
DB_CFG = dict(
    host="localhost",
    port=5432,
    database="moja_db",
...
def resolve_desktop_path(user_input: str | None) -> Path:
    ...
    if user_input:
        ...
        p = Path(raw)
        return p if p.is_absolute() else (desktop / p.name)

    imgs = []
    for ext in ("*.png","*.jpg","*.jpeg","*.webp","*.jfif"):
        imgs += list(desktop.glob(ext))
    if not imgs:
        print("❌ Na Desktope som nenašiel žiadne obrázky. Zadaj cestu k súboru:")
        p = input("> ").strip()
        return resolve_desktop_path(p or None)

    imgs.sort(key=lambda x: x.stat().st_mtime, reverse=True)
    print("Na Desktope som našiel tieto obrázky (naposledy upravené hore):")
    for i, f in enumerate(imgs[:10], 1):
        print(f"  {i}. {f.name}")
    ch = input("Zadaj názov súboru (alebo prázdne pre 1.): ").strip()
    return (desktop / ch) if ch else imgs[0]
...
def detect_waste_code(image_path: Path) -> int | None:
    """
    IBA EasyOCR:
      1) nájdi štítok a jeho bbox,
      2) číslo čítaj len v páse nad štítkom,
      3) hlasovanie z variantov + heuristiky,
      4) štítok (conf>=0.60) má prioritu.
    """
    ...
    if label_key and label_conf >= LABEL_CONF_OK:
        # rýchle mapovanie „istých“ štítkov
        # (PAP a GL majú defaulty; ak chceš 20/21/22 alebo 70/71/72, čítame číslo nižšie)
        ...
        # pre PAP/GL/kompozity sa môžeme pokúsiť prečítať konkrétne číslo nižšie
        # ale ak nie, vrátia sa defaulty nižšie

    # --- 2) urči ROI nad štítkom (alebo stredný pás), číslo čítaj len tam ---
    if label_box:
    ...
    # --- 4) Heuristiky a defaulty podľa štítku ---
    if label_key == "PAP":
        # ak sme našli číslo 20/21/22, vráť ho; inak default 22
    ...
    # nemáme spoľahlivé číslo → ak aspoň nejaký štítok, vráť default rodiny
...
def main():
    ...
    parser.add_argument("image", nargs="?", help="Cesta k obrázku alebo názov súboru z Desktopu.")
